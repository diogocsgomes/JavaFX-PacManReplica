...
public class OngoingGameState extends GameStateAdapter {

    GameState state = GameState.ONGOING_GAME;

    private GhostMovement ghostMovement;
    public OngoingGameState(GameData GameEntities, Context context,GhostMovement GhostMovement){
    ...
    @Override
    public char[][] getOutput() { //Function not being USED!!!!!
            ...
            for (x = 0; x < board[0].length; x++) {
                ...
                && (y != CordPacY || x != CordPacx - 1) && (y != CordPacY || x != CordPacx + 1)) // verifica se e a outra zona Warp
                {
                    ...
                    if (!HelpFunctions.detectWallORGhostPortal(y, x + 1, board)) // se for para a direita , por outras palavras, se para direita form uma direcao valida
                    {
                        ...
                        GameEntities.getPacMan().setDirection(RIGHT);
                        return GameEntities.getBoard();
                    }
